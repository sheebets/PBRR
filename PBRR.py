...
def main():
    ...
    # Buttons section - conditional display
    if 'schedule' not in st.session_state:
        # Before generating: Show only Generate button
        ...
        # After generating: Show only Re-run and Reset buttons
        col1, col2 = st.columns(2)
        
        with col1:
            if st.button("▶️ Re-run", help="Re-run based on scores", use_container_width=True):
        ...
        with col2:
            if st.button("🔁 Reset", help="Start over", use_container_width=True):
    ...
    # Display results only if schedule exists
